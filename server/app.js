 ...
 *
 * Separate from index.js so tests can boot it on an ephemeral port and make
 * real requests, rather than the routes being the one part of the server with
 * no coverage.
 */
export function createApp() {
  ...
  app.use(
    cors({
      origin(origin, callback) {
        ...
        // probe. Those are not browser cross-origin calls, so there is nothing
        // to block.
  ...
  // Health and airport lookup answer before the broad limiter: neither spends
  // anything upstream, so neither should eat the allowance the paid endpoints
  // rely on.
...
/** Registered here so the app is complete however it is started. */
export function registerProviders() {
  return registerDefaultProviders();
}
